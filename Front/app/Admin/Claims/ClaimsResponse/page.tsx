...
const ClaimsResponse  = () => {
  ...
  const [claimRes,setClaimRes] = useState<Claim[]>([]) //All Claims sent to the admin//
...
// Get all claims sent to the admin //
  const fetchData = async (idadmin : number) => {
    try {
      const res = await fetch(`http://localhost:3000/api/getClaimsRespons/2`);
...
<button className='shadow-xl rounded w-32 h-12 font-bold  bg-gray-200 justify-center hover:bg-gray-300 ml-1'><Link href="/Admin/Claims"> My ClaimInbox </Link> </button>
